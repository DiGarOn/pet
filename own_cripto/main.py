...
# ключ состоит пары: Буква, одно число из: [1; 2; 3; 4; 5; 6; 7; 8]. тогда всего ключей
# ровно (32*8). далее создаются случайным образом ключи. число ключей равно len(text)//3 + 1(если не делится нацело)
# далее происходит зашифрование методом диска Альберти:
# для каждого блока из 3х букв есть свой ключ, из цифры в ключе формируется внутренний диск альберти:
#    1 цикл по предыдущему внутреннему кругу, шаг = числу, в каждый момент буква из алфавита переносится
#    в начало строки
#    далее внутренний диск устанавливается буквой из ключа напротив буквы А внешнего диска
# Далее каждая БУКВА из блока данных кодируется, диск поворачивается на число из ключа по часовой, если
# ASCII(буквы из ключа) * число из ключа четно, иначе против часовой.
# текст дополняется пробелами в конец, если не делится на блоки по 3 символа.
# в конец зашифрованного текста записываются все ключи так, что буква и число увеличиваются на 1
...
def decript(text):
    ...
    for i in range(len(text) - 1, 0, -2):
        if text[i] in '1234567890':
            keys.insert(0, [chr(ord(text[i - 1]) - 1), int(text[i]) - 1])
        else:
            end_key = i
            break
    text = text[:end_key + 1]
...
def main():
    ...
    enc_text = encript(text.upper())
    print(enc_text)
    print('________________________________________________________________________')
    print(decript(enc_text))
    print('________________________________________________________________________')
    ...
    enc_text_1 = encript(text_1.upper())
    print(enc_text_1)
    print('________________________________________________________________________')
    print(decript(enc_text_1))
    print('________________________________________________________________________')
    ...
    enc_text_2 = encript(text_2.upper())
    print(enc_text_2)
    print('________________________________________________________________________')
    print(decript(enc_text_2))
    print('________________________________________________________________________')
    ...
    enc_text_3 = encript(text_3.upper())
    print(enc_text_3)
    print('________________________________________________________________________')
    print(decript(enc_text_3))
    print('________________________________________________________________________')
